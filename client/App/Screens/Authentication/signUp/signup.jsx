...
const Signup = ({ navigation }) => {
  ...
  const createCompanyAcc = async () => {
    ...
    try {
      const emailSent = await axios.post(
        `http://${process.env.EXPO_PUBLIC_IP_KEY}:4070/api/company/create`,
        { email, name, pdp, userName }
      );
      const response = await axios.post(
        `http://${process.env.EXPO_PUBLIC_IP_KEY}:4070/api/user/create`,
        companyData
      );
  ...
  const addSpeciality = async (id) => {
    ...
    try {
      await axios.post(
        `http://${process.env.EXPO_PUBLIC_IP_KEY}:4070/technologie/user/speciality/add`,
        obj
      );
    } catch (err) {
      console.log(err);
  ...
  const addTechnologies = async (id) => {
    const data = technologies.map((e) => ({
      technologiesId: e.id,
      userId: id,
    }));
    try {
      await axios.post(
        `http://${process.env.EXPO_PUBLIC_IP_KEY}:4070/technologie/user/Technology/${id}`,
        { data: data }
      );
      ...
      <SafeAreaView style={{}}>
        <View>
              ...
              transform: [
                {
                  translateX: slideAnim.interpolate({
                    ...
                    outputRange: [0, 400, 800, 1200, 1600],
                  }),
                },
              ],
            }}
          >
            ...
            {step === 3 && (
              <Step3 setLocation={setLocation} setStep={setStep} />
            )}
            {step === 4 && (
              <Step4 setSpeciality={setSpeciality} setStep={setStep} />
            )}
            {step === 5 && (
              <Step5
